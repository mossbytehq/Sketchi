...
pub struct WinitSeatState {
    /// The data device used to receive native Wayland drag-and-drop offers.
...
impl WinitSeatState {
    pub fn new() -> Self {
        Self { data_device: None, ..Default::default() }
    }
}
...
impl SeatHandler for WinitState {
    ...
    fn new_capability(
        ...
        if self
            .seats
            .get(&seat.id())
            .is_some_and(|seat_state| seat_state.data_device.is_none())
        {
            let data_device = self
                .data_device_manager_state
                .as_ref()
                .map(|manager| manager.get_data_device(queue_handle, &seat));
            if let Some(seat_state) = self.seats.get_mut(&seat.id()) {
                seat_state.data_device = data_device;
            }
        }
    ...
    fn new_seat(
        ...
        let data_device = self
            ...
            .as_ref()
            .map(|manager| manager.get_data_device(queue_handle, &seat));
        self.seats.insert(seat.id(), WinitSeatState { data_device, ..WinitSeatState::new() });
    }
